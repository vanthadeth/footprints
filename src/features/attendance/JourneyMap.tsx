...
/**
 * Personal journey map: each visit as a numbered green pin, plus clock-in
 * ("IN", brand blue) and clock-out ("OUT", earth) pins for every attendance
 * session that day, all connected chronologically (spec §32) -- so the
 * route shown here matches the same day's timeline: where the day started,
 * every stop in between, and where it ended (or last ended, if clocked out
 * and back in more than once). Points with no recorded position (shouldn't
 * happen -- GPS is required at check-in/clock-in -- but defends against bad
 * data) are simply skipped rather than crashing the map.
 */
  ...
  return (
    <MapView points={points} height={height}>
      ...
      {items.map((p) => {
        if (p.kind === 'clock-in') {
          return (
            <Marker key={`in-${p.session.id}`} position={[p.lat, p.lng]} icon={pinIcon('#1668b8', { label: 'IN' })}>
              <Popup>
        ...
        if (p.kind === 'clock-out') {
          ...
          return (
            <Marker key={`out-${session.id}`} position={[p.lat, p.lng]} icon={pinIcon('#96703f', { label: 'OUT' })}>
              <Popup>
                <div className="text-sm">
        ...
        const visit = p.visit
        return (
          <Marker key={visit.id} position={[p.lat, p.lng]} icon={pinIcon('#0f6e4f', { label: String(p.index + 1) })}>
            <Popup>
              <div className="text-sm">
